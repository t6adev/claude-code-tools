...
| [`project/templates/`](project/templates/) | プロジェクト種別ごとの CLAUDE.md テンプレート |
...
Skills・Agents をグローバル（`~/.claude/`）へシンボリックリンクし、Hooks・MCP 設定をコピー、推薦 plugins を `claude plugin install` でインストールします。
...
~/path/to/claude-code-tools/install.sh --no-skills --no-agents --no-hooks --no-mcp
```
...
利用可能なテンプレート一覧は [`project/templates/README.md`](project/templates/README.md) を参照。
