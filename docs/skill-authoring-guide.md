...
あるスキルの特定ステップで別のスキルが有用な場合、**ハード依存を持たず**以下の形式でヒントを添える:

```
（`/skill-name` が利用可能な場合は、〜するとよい）
```

- 括弧で囲み「補足情報」であることを示す
- バッククォート + スラッシュコマンド形式を統一する
- 「利用可能な場合は」で条件を明示する
- 「〜するとよい」（ソフト推薦）vs「〜する」（必須）の文法的区別を守る
- スキルが未インストールでも前後の本文だけで動作するよう、意図をインラインで短く書く
