...
デフォルトの親ディレクトリはスキル実行ディレクトリの1階層上に `repro-<問題名>/` として作成する。
...
| パターン       | 結果     | 考察                         |
|---------------|----------|------------------------------|
| baseline      | ❌ 再現  | 最小構成でも再現             |
| version-a     | ✅ 正常  | v2.0.0 では発生しない        |
| version-b     | ❌ 再現  | v3.0.0 で発生                |
| no-plugin-x   | ✅ 正常  | plugin-x が原因の可能性      |
```

結果から原因の仮説を提示し、次のアクション（Issue 報告、ワークアラウンド、修正）を提案する。
