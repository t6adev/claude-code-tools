...
同時に起動する Agent 数は **最大 3** に制限する。
API レートリミットを回避するため、レベル内の Issue が 3 を超える場合はバッチに分割し、
前のバッチの Agent がすべて完了してから次のバッチを起動する。

```
例: Level 0 に 5 件ある場合
  バッチ 1: #10, #11, #12（3 並列）→ 全完了を待つ
  バッチ 2: #13, #14（2 並列）
```

### 並列実行の仕組み

同一バッチ内の各 Issue に対して、Agent tool を `run_in_background: true` で起動する。
**1 つのメッセージ内で、同一バッチの全 Issue に対する Agent 呼び出しをまとめて送信する。**
