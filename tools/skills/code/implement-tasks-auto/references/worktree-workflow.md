...
`<worktree-dir>` はブランチ名の `/` を `-` に置換した文字列**のみ**で構成する。親ディレクトリ名などの余計な階層を追加してはならない。
...
**パス解決の例:** リポジトリが `~/workspace/app-worktree/app` にある場合:

```
cwd:    ~/workspace/app-worktree/app
../   = ~/workspace/app-worktree/
結果:   ~/workspace/app-worktree/<worktree-dir>
```

`../app-worktree/<worktree-dir>` のように親ディレクトリ名を含めてはならない。
